...
// javaGenPkgOutDir is the directory under the package directory where generated files will be placed.
var javaGenPkgOutDir string
...
// SetJavaGenPkgOutDir sets the directory under the package directory where generated files will be placed.
func SetJavaGenPkgOutDir(dir string) {
	javaGenPkgOutDir = dir
}

// javaGenPkgPath returns the Java package path given the Go package path.
func javaGenPkgPath(goPkgPath string) string {
	pathSuffix := goPkgPath
	if javaGenPkgOutDir != "" {
		pathSuffix = path.Join(pathSuffix, javaGenPkgOutDir)
	}
	return path.Join(javaGenPkgPrefix, pathSuffix)
}
...
public class Client { {{range $file := $data.Package.Files}}
	/* Bind methods for interfaces in file: {{$file.BaseName}}. */{{range $iface := $file.Interfaces}}{{$ifaceClassName := javaIfaceName $iface (not $isService) $data.Imports}}{{$ifaceShortClassName := javaShortIfaceName $iface (not $isService)}}
	public static {{$ifaceClassName}} bind{{$ifaceShortClassName}}({{$stringClassName}} name) throws {{$veyronExceptionClassName}} {
		return bind{{$ifaceShortClassName}}(name, null);
	}
	public static {{$ifaceClassName}} bind{{$ifaceShortClassName}}({{$stringClassName}} name, {{$optsClassName}} {{$optionVarName}}) throws {{$veyronExceptionClassName}} {
		...
		}{{range $eIface := $iface.Embeds}}{{$eIfaceClassName := javaIfaceName $eIface (not $isService) $data.Imports}}{{$eIfaceShortClassName := javaShortIfaceName $eIface (not $isService)}}
		final {{$eIfaceClassName}} {{javaClassToVarName $eIfaceShortClassName}} = {{javaName (printf "%s/Client" (javaGenPkgPath $eIface.File.Package.Path)) $data.Imports}}.bind{{$eIface.Name}}(name, {{$optionVarName}});{{end}}
		return new {{$ifaceShortClassName}}Stub(client, name{{range $eIface := $iface.Embeds}}{{$eIfaceShortClassName := javaShortIfaceName $eIface (not $isService)}}, {{javaClassToVarName $eIfaceShortClassName}}{{end}});
	}{{end}}{{end}}
{{range $file := $data.Package.Files}}
	/* Client stubs for interfaces in file: {{$file.BaseName}}. */{{range $iface := $file.Interfaces}}{{$ifaceClassName := javaIfaceName $iface (not $isService) $data.Imports}}{{$ifaceShortClassName := javaShortIfaceName $iface (not $isService)}}
	private static class {{$ifaceShortClassName}}Stub implements {{$ifaceClassName}} {
		private static final {{$stringClassName}} vdlIfacePathOpt = "{{javaPath (javaGenPkgPath $data.Package.Path)}}.{{$ifaceShortClassName}}";
		...
		{{$ifaceShortClassName}}Stub({{$clientClassName}} client, {{$stringClassName}} name{{range $eIface := $iface.Embeds}}{{$eIfaceClassName := javaIfaceName $eIface (not $isService) $data.Imports}}{{$eIfaceShortClassName := javaShortIfaceName $eIface (not $isService)}}, {{$eIfaceClassName}} {{javaClassToVarName $eIfaceShortClassName}}{{end}}) {
			this.client = client;
...
public class Server { {{range $file := $data.Package.Files}}
	/* Server stub creation methods for interfaces in file: {{$file.BaseName}}. */{{range $iface := $file.Interfaces}}{{$ifaceClassName := javaIfaceName $iface $isService $data.Imports}}{{$ifaceShortClassName := javaShortIfaceName $iface (not $isService)}}
	public static {{$objectClassName}} new{{$iface.Name}}({{$ifaceClassName}} service) { {{range $eIface := $iface.Embeds}}{{$eIfaceClassName := javaIfaceName $eIface $isService $data.Imports}}{{$eIfaceShortClassName := javaShortIfaceName $eIface (not $isService)}}{{$eIfaceServerClassName := javaQualifiedName "Server" $eIface.File $data.Imports}}{{$eIfaceStubClassName := printf "%s.%sStub" $eIfaceServerClassName $eIface.Name}}
		final {{$eIfaceStubClassName}} {{javaClassToVarName $eIfaceShortClassName}} = ({{$eIfaceStubClassName}}){{$eIfaceServerClassName}}.new{{$eIface.Name}}(service);{{end}}
		return new {{$iface.Name}}Stub(service{{range $eIface := $iface.Embeds}}{{$eIfaceShortClassName := javaShortIfaceName $eIface (not $isService)}}, {{javaClassToVarName $eIfaceShortClassName}}{{end}});
	}{{end}}{{end}}
	{{range $file := $data.Package.Files}}
	/* Server stubs for interfaces in file: {{$file.BaseName}}. */{{range $iface := $file.Interfaces}}{{$ifaceClassName := javaIfaceName $iface $isService $data.Imports}}{{$ifaceShortClassName := javaShortIfaceName $iface (not $isService)}}
	public static class {{$iface.Name}}Stub {
		private final {{$ifaceClassName}} service;{{range $eIface := $iface.Embeds}}{{$eIfaceClassName := javaIfaceName $eIface $isService $data.Imports}}{{$eIfaceShortClassName := javaShortIfaceName $eIface (not $isService)}}{{$eIfaceServerClassName := javaQualifiedName "Server" $eIface.File $data.Imports}}{{$eIfaceStubClassName := printf "%s.%sStub" $eIfaceServerClassName $eIface.Name}}
		private final {{$eIfaceStubClassName}} {{javaClassToVarName $eIfaceShortClassName}};{{end}}

		{{$iface.Name}}Stub({{$ifaceClassName}} service{{range $eIface := $iface.Embeds}}{{$eIfaceClassName := javaIfaceName $eIface $isService $data.Imports}}{{$eIfaceShortClassName := javaShortIfaceName $eIface (not $isService)}}{{$eIfaceServerClassName := javaQualifiedName "Server" $eIface.File $data.Imports}}{{$eIfaceStubClassName := printf "%s.%sStub" $eIfaceServerClassName $eIface.Name}}, {{$eIfaceStubClassName}} {{javaClassToVarName $eIfaceShortClassName}}{{end}}) {
			this.service = service;{{range $eIface := $iface.Embeds}}{{$eIfaceShortClassName := javaShortIfaceName $eIface (not $isService)}}
			this.{{javaClassToVarName $eIfaceShortClassName}} = {{javaClassToVarName $eIfaceShortClassName}};{{end}}
		}
		/**
...
func javaPackageDefinedPaths(pkg *compile.Package) []string {
	...
	paths := []string{
		path.Join(pkgPath, "Client"),
		path.Join(pkgPath, "Server"),
		path.Join(pkgPath, "Consts"),
	}
...
func genJavaUserImports(classPaths []string, pkg *compile.Package) (ret javaUserImports) {
	...
	for _, class := range classPaths {
		...
		case path.Dir(class) == javaGenPkgPath(pkg.Path):
			ret[path.Base(class)] = path.Dir(class)
		case path.Dir(class) == "java/lang" && cur != javaGenPkgPath(pkg.Path):
			ret[path.Base(class)] = path.Dir(class)
		}
	}
	return
...
func javaInArgs(method *compile.Method, iface *compile.Interface, isService, useOptions, camelCase bool, imports javaUserImports, env *compile.Env) (ret string) {
	...
	for _, arg := range method.InArgs {
		...
		argName := arg.Name
		if camelCase {
			argName = util.ToCamelCase(argName)
		}
...
func javaClassToVarName(className string) string {
	...
	if dir == "." {
		return util.ToCamelCase(className)
	}
	return strings.Replace(path.Dir(className), "/", "_", -1) + "_" + util.ToCamelCase(path.Base(className))
}
