...
public class Client { {{range $file := $data.Package.Files}}
	...
	public static {{$ifaceClassName}} bind{{$ifaceClassName}}({{$stringClassName}} name) throws {{$veyronExceptionClassName}} {
		return bind{{$ifaceClassName}}(name, null);
	}
	public static {{$ifaceClassName}} bind{{$ifaceClassName}}({{$stringClassName}} name, {{$optsClassName}} opts) throws {{$veyronExceptionClassName}} {
		...
		}{{range $eIface := $iface.Embeds}}{{$eIfaceClassName := javaIfaceName $eIface (not $isService) $data.Imports}}
...
{{range $file := $data.Package.Files}}
	...
	private static class {{$ifaceClassName}}Stub implements {{$ifaceClassName}} {
		private static final {{$stringClassName}} vdlIfacePathOpt = "{{javaPath (javaGenPkgPath $data.Package.Path)}}.{{$ifaceClassName}}";
		...
		public {{$outArgType}} {{toCamelCase $method.Name}}({{javaInArgs $method $iface (not $isService) $useOptions (not $camelCase) $data.Imports $data.Env}}) throws {{$veyronExceptionClassName}} {
			...
			if (opts == null) opts = new {{$optsClassName}}();
			if (!opts.has({{$optDefsClassName}}.VDL_INTERFACE_PATH)) {
				opts.set({{$optDefsClassName}}.VDL_INTERFACE_PATH, {{$ifaceClassName}}Stub.vdlIfacePathOpt);
			}
...
{{if javaIsStreamingMethod $method}}{{$typeTokenClassName := javaName "com/google/common/reflect/TypeToken" $data.Imports}}{{$streamClassName := javaName "com/veyron2/vdl/ClientStream" $data.Imports}}{{$streamSend := javaType $method.InStream $forceClass $data.Imports $data.Env}}{{$streamRecv := javaType $method.OutStream $forceClass $data.Imports $data.Env}}{{$streamFinish := javaNonStreamingOutArgType $method $iface (not $isService) $forceClass $data.Imports $data.Env}}
		...
		public {{$outArgType}} {{toCamelCase $method.Name}}({{javaInArgs $method $iface (not $isService) $useOptions (not $camelCase) $data.Imports $data.Env}}) throws {{javaName "com/veyron2/ipc/VeyronException" $data.Imports}} {
			...
			{{if ne $outArgType "void"}}return {{end}}this.{{javaClassToVarName $eIfaceClassName}}.{{toCamelCase $method.Name}}(context{{range $arg := $method.InArgs}}, {{$arg.Name}}{{end}}, opts);
		}{{end}}{{end}}
	}{{end}}{{end}}
...
func javaInArgs(method *compile.Method, iface *compile.Interface, isService, useOptions, camelCase bool, imports javaUserImports, env *compile.Env) (ret string) {
	...
	if !isService && useOptions {
		...
		ret += fmt.Sprintf("%s opts", javaName("com/veyron2/Options", imports))
	}
	if javaIsStreamingMethod(method) && isService {
