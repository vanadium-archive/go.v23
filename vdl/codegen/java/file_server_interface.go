...
{{ range $method := .Methods }}
    ...
    {{ $method.Doc }}
    {{ $method.AccessModifier }} {{ $method.RetType }} {{ $method.Name }}(final io.v.core.veyron2.ipc.ServerContext context{{ $method.Args }}) throws io.v.core.veyron2.VeyronException;
{{ end }}
}
`
...
func processServerInterfaceMethod(method *compile.Method, iface *compile.Interface, env *compile.Env) serverInterfaceMethod {
	...
	if isStreamingMethod(method) {
		args += fmt.Sprintf(", io.v.core.veyron2.vdl.Stream<%s, %s> stream", javaType(method.OutStream, true, env), javaType(method.InStream, true, env))
	}
