...
public final class {{ .ServiceName }}ClientStub implements {{ .FullServiceName }}Client {
...
func genJavaClientStubFile(iface *compile.Interface, env *compile.Env) JavaFileInfo {
	...
	for _, embed := range allEmbeddedIfaces(iface) {
		embeds = append(embeds, clientStubEmbed{
			...
			StubClassName:    javaPath(javaGenPkgPath(path.Join(embed.File.Package.Path, javaGenImplDir, embed.Name+"ClientStub"))),
		})
	}
	...
	return JavaFileInfo{
		Dir:  javaGenImplDir,
		Name: iface.Name + "ClientStub.java",
		Data: buf.Bytes(),
	}
}
