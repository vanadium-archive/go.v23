...
// Ciphertext represents the ciphertext generated by a BlessingsBasedEncrypter.
...
func (Ciphertext) __VDLReflect(struct {
	Name string `vdl:"v.io/v23/security.Ciphertext"`
}) {
}
...
// IBE is a cryptographic scheme for blessing pattern encryption that uses
// identity-based encryption (IBE) with blessing patterns as identities.
//
// Encrypting a message under a blessing pattern involves using an identity-based
// encryption scheme (e.g., Boneh-Boyen scheme) with the pattern as the identity.
//
// A blessing is associated with a set of identity private keys -- one for each pattern
// matched by the blessing. Decrypting a message involves choosing the appropriate
// private key associated with a blessing matching the pattern used in the ciphertext.
//
// For space-efficiency, the ciphertext only mentions a 16 byte truncated hash of the
// pattern.
const IBE = int32(1)
