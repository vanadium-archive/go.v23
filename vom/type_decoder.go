...
func NewTypeDecoder(r io.Reader) *TypeDecoder {
	return &TypeDecoder{
		...
		dec:                 newDecoderWithoutVersionByte(newDecbuf(r), nil),
		receivedVersionByte: false,
	}
}

func newTypeDecoderWithoutVersionByte(buf *decbuf) *TypeDecoder {
	return &TypeDecoder{
		idToType:            make(map[typeId]*vdl.Type),
		idToWire:            make(map[typeId]wireType),
		dec:                 newDecoderWithoutVersionByte(buf, nil),
		receivedVersionByte: true,
	}
}
...
func (d *TypeDecoder) readWireType(tid typeId) error {
	...
	if !d.receivedVersionByte {
		if err := readVersionByte(d.dec.buf); err != nil {
			return err
		}
		d.receivedVersionByte = true
	}
