...
func runDecode(cmd *cmdline.Command, args []string) error {
	...
	decoder, err := vom2.NewDecoder(reader)
	if err != nil {
		return err
	}
...
func runDump(cmd *cmdline.Command, args []string) error {
	...
	case len(args) == 1:
		...
		fmt.Fprintln(cmd.Stdout(), vom2.Dump(binbytes))
		return nil
	}
	// Handle streaming from stdin.
	// TODO(toddw): Add a flag to configure stdout/stderr dumping.
	dumper := vom2.NewDumper(dumpWriter{cmd.Stdout(), cmd.Stdout()})
	defer dumper.Close()
...
func runDumpHexStream(dumper *vom2.Dumper) error {
	buf := make([]byte, 1024)
	bufStart := 0
ReadLoop:
	for {
...
var _ vom2.DumpWriter = dumpWriter{}

func (w dumpWriter) WriteAtom(atom vom2.DumpAtom) {
	w.atom.Write([]byte(atom.String() + "\n"))
}

func (w dumpWriter) WriteStatus(status vom2.DumpStatus) {
	w.status.Write([]byte("\n" + status.String() + "\n"))
}
