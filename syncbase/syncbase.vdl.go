	...
	*x = Conflict{}
	...
	for {
		...
		case "ReadSet":
			...
			if dec.IsNil() {
				if !vdl.Compatible(dec.Type(), vdl.TypeOf(x.ReadSet)) {
		...
		case "WriteSet":
			...
			if dec.IsNil() {
				if !vdl.Compatible(dec.Type(), vdl.TypeOf(x.WriteSet)) {
		...
		case "ScanSet":
			...
			if dec.IsNil() {
				if !vdl.Compatible(dec.Type(), vdl.TypeOf(x.ScanSet)) {
	...
	*x = ResolvedRow{}
	...
	for {
		...
		case "Result":
			...
			if dec.IsNil() {
				if !vdl.Compatible(dec.Type(), vdl.TypeOf(x.Result)) {
