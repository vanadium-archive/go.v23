...
func Eval(db query.Database, k string, v *vdl.Value, e *query_parser.Expression) bool {
...
func evalLogicalOperators(db query.Database, k string, v *vdl.Value, e *query_parser.Expression) bool {
...
func evalComparisonOperators(db query.Database, k string, v *vdl.Value, e *query_parser.Expression) bool {
...
func resolveArgsAndExecFunction(db query.Database, k string, v *vdl.Value, f *query_parser.Function) (*query_parser.Operand, error) {
	// Resolve the function's arguments
	callingArgs := []*query_parser.Operand{}
	for _, arg := range f.Args {
		resolvedArg := resolveOperand(db, k, v, arg)
		if resolvedArg == nil {
			return nil, query.NewErrFunctionArgBad(db.GetContext(), arg.Off, f.Name, arg.String())
		}
...
func resolveOperand(db query.Database, k string, v *vdl.Value, o *query_parser.Operand) *query_parser.Operand {
...
func resolveWithKey(db query.Database, k string, v *vdl.Value, object *vdl.Value, segment query_parser.Segment) *vdl.Value {
...
// Resolve a field.
func ResolveField(db query.Database, k string, v *vdl.Value, f *query_parser.Field) *vdl.Value {
...
func EvalWhereUsingOnlyKey(db query.Database, s *query_parser.SelectStatement, k string) EvalWithKeyResult {
...
func evalExprUsingOnlyKey(db query.Database, e *query_parser.Expression, k string) EvalWithKeyResult {
	switch e.Operator.Type {
	case query_parser.And:
