...
type queryFunc func(query.Database, int64, []*query_parser.Operand) (*query_parser.Operand, error)
type checkArgsFunc func(query.Database, int64, []*query_parser.Operand) error
...
func CheckFunction(db query.Database, f *query_parser.Function) error {
	if entry, err := lookupFuncName(db, f); err != nil {
		...
		if !entry.hasVarArgs && len(f.Args) != len(entry.argTypes) {
			return query.NewErrFunctionArgCount(db.GetContext(), f.Off, f.Name, int64(len(f.ArgTypes)), int64(len(f.Args)))
		}
		if entry.hasVarArgs && len(f.Args) < len(entry.argTypes) {
			return query.NewErrFunctionAtLeastArgCount(db.GetContext(), f.Off, f.Name, int64(len(f.ArgTypes)), int64(len(f.Args)))
		}
...
func lookupFuncName(db query.Database, f *query_parser.Function) (*function, error) {
	if entry, ok := functions[f.Name]; !ok {
		...
		if correctCase, ok := lowercaseFunctions[strings.ToLower(f.Name)]; !ok {
			return nil, query.NewErrFunctionNotFound(db.GetContext(), f.Off, f.Name)
		} else {
			// the case is wrong
			return nil, query.NewErrDidYouMeanFunction(db.GetContext(), f.Off, correctCase)
		}
	} else {
		return &entry, nil
	}
...
func FuncCheck(db query.Database, f *query_parser.Function, args []*query_parser.Operand) error {
	if entry, err := lookupFuncName(db, f); err != nil {
		return err
	} else {
...
func ExecFunction(db query.Database, f *query_parser.Function, args []*query_parser.Operand) (*query_parser.Operand, error) {
	if entry, err := lookupFuncName(db, f); err != nil {
		return nil, err
	} else {
...
func checkArg(db query.Database, off int64, argType query_parser.OperandType, arg *query_parser.Operand) error {
	...
	case query_parser.TypBigInt:
		...
		if err != nil {
			err = query.NewErrBigIntConversionError(db.GetContext(), arg.Off, err)
		}
	case query_parser.TypBigRat:
		_, err = conversions.ConvertValueToBigRat(operandToConvert)
		if err != nil {
			err = query.NewErrBigRatConversionError(db.GetContext(), arg.Off, err)
		}
	case query_parser.TypBool:
		_, err = conversions.ConvertValueToBool(operandToConvert)
		if err != nil {
			err = query.NewErrBoolConversionError(db.GetContext(), arg.Off, err)
		}
	case query_parser.TypComplex:
		_, err = conversions.ConvertValueToComplex(operandToConvert)
		if err != nil {
			err = query.NewErrComplexConversionError(db.GetContext(), arg.Off, err)
		}
	case query_parser.TypFloat:
		_, err = conversions.ConvertValueToFloat(operandToConvert)
		if err != nil {
			err = query.NewErrFloatConversionError(db.GetContext(), arg.Off, err)
		}
	case query_parser.TypInt:
		_, err = conversions.ConvertValueToInt(operandToConvert)
		if err != nil {
			err = query.NewErrIntConversionError(db.GetContext(), arg.Off, err)
		}
	case query_parser.TypStr:
		_, err = conversions.ConvertValueToString(operandToConvert)
		if err != nil {
			err = query.NewErrStringConversionError(db.GetContext(), arg.Off, err)
		}
	case query_parser.TypTime:
		_, err = conversions.ConvertValueToTime(operandToConvert)
		if err != nil {
			err = query.NewErrTimeConversionError(db.GetContext(), arg.Off, err)
		}
	case query_parser.TypUint:
		_, err = conversions.ConvertValueToUint(operandToConvert)
		if err != nil {
			err = query.NewErrUintConversionError(db.GetContext(), arg.Off, err)
		}
	}
	return err
}
...
func argsStandardCheck(db query.Database, off int64, f *function, args []*query_parser.Operand) error {
