...
import (
	...
	"v.io/v23/syncbase/nosql/query/internal/query_functions"
...
var (
	KeyRangeAll = query.KeyRange{Start: "", Limit: MaxRangeLimit}
)

func Check(db query.Database, s *query_parser.Statement) error {
	switch sel := (*s).(type) {
	case query_parser.SelectStatement:
		return checkSelectStatement(db, &sel)
	default:
		return query.NewErrCheckOfUnknownStatementType(db.GetContext(), (*s).Offset())
	}
}

func checkSelectStatement(db query.Database, s *query_parser.SelectStatement) error {
...
func checkSelectClause(db query.Database, s *query_parser.SelectClause) error {
		...
		case query_parser.TypSelField:
			...
			case "k":
				if len(selector.Field.Segments) > 1 {
					return query.NewErrDotNotationDisallowedForKey(db.GetContext(), selector.Field.Segments[1].Off)
				}
			case "v":
				// Nothing to check.
			case "K":
				// Be nice and warn of mistakenly capped 'K'.
				return query.NewErrDidYouMeanLowercaseK(db.GetContext(), selector.Field.Segments[0].Off)
			case "V":
				// Be nice and warn of mistakenly capped 'V'.
				return query.NewErrDidYouMeanLowercaseV(db.GetContext(), selector.Field.Segments[0].Off)
			default:
				return query.NewErrInvalidSelectField(db.GetContext(), selector.Field.Segments[0].Off)
			}
...
func checkFromClause(db query.Database, f *query_parser.FromClause) error {
	var err error
	f.Table.DBTable, err = db.GetTable(f.Table.Name)
	if err != nil {
		return query.NewErrTableCantAccess(db.GetContext(), f.Table.Off, f.Table.Name, err)
	}
	return nil
}

// Check where clause.
func checkWhereClause(db query.Database, w *query_parser.WhereClause) error {
	if w == nil {
		return nil
	}
...
func checkExpression(db query.Database, e *query_parser.Expression) error {
	...
	if e.Operator.Type == query_parser.Like || e.Operator.Type == query_parser.NotLike {
		if e.Operand2.Type != query_parser.TypStr {
			return query.NewErrLikeExpressionsRequireRhsString(db.GetContext(), e.Off)
		}
	...
	if e.Operator.Type == query_parser.Is || e.Operator.Type == query_parser.IsNot {
		if !IsField(e.Operand1) && !IsFunction(e.Operand1) {
			return query.NewErrIsIsNotRequireLhsValue(db.GetContext(), e.Operand1.Off)
		}
		if e.Operand2.Type != query_parser.TypNil {
			return query.NewErrIsIsNotRequireRhsNil(db.GetContext(), e.Operand2.Off)
		}
	}
	...
	if ContainsKeyOperand(e) && ((isLiteral(e.Operand1) && !isStringLiteral(e.Operand1)) ||
		...
		return query.NewErrKeyExpressionLiteral(db.GetContext(), off)
	}
	...
	if (e.Operand1.Type == query_parser.TypBool || e.Operand2.Type == query_parser.TypBool) && e.Operator.Type != query_parser.Equal && e.Operator.Type != query_parser.NotEqual {
		return query.NewErrBoolInvalidExpression(db.GetContext(), e.Operator.Off)
	}

	return nil
}

func checkOperand(db query.Database, o *query_parser.Operand) error {
	switch o.Type {
	...
	case query_parser.TypField:
		...
		case "k":
			if len(o.Column.Segments) > 1 {
				return query.NewErrDotNotationDisallowedForKey(db.GetContext(), o.Column.Segments[1].Off)
			}
		case "v":
			// Nothing to do.
		case "K":
			// Be nice and warn of mistakenly capped 'K'.
			return query.NewErrDidYouMeanLowercaseK(db.GetContext(), o.Column.Segments[0].Off)
		case "V":
			// Be nice and warn of mistakenly capped 'V'.
			return query.NewErrDidYouMeanLowercaseV(db.GetContext(), o.Column.Segments[0].Off)
		default:
			return query.NewErrBadFieldInWhere(db.GetContext(), o.Column.Segments[0].Off)
		}
		return nil
...
func computePrefix(db query.Database, off int64, s string) (string, error) {
	...
	for _, c := range s {
		if escapedChar {
			...
			case '_':
				s2 += string(c)
			default:
				return "", query.NewErrInvalidEscapedChar(db.GetContext(), off)
			}
			escapedChar = false
		} else {
			if c == '%' || c == '_' {
...
func computeRegex(db query.Database, off int64, s string) (string, *regexp.Regexp, bool, error) {
	...
	if err != nil {
		return "", nil, false, query.NewErrErrorCompilingRegularExpression(db.GetContext(), off, regex, err)
	}
...
func computeKeyRangeForLike(prefix string) query.KeyRange {
	if prefix == "" {
		return KeyRangeAll
	}
	return query.KeyRange{Start: prefix, Limit: afterPrefix(prefix)}
}

func computeKeyRangesForNotLike(prefix string) *query.KeyRanges {
	if prefix == "" {
		return &query.KeyRanges{KeyRangeAll}
	}
	return &query.KeyRanges{
		query.KeyRange{Start: "", Limit: prefix},
		query.KeyRange{Start: afterPrefix(prefix), Limit: ""},
	}
}
...
func computeKeyRangeForSingleValue(start string) query.KeyRange {
	limit := []byte(start)
	limit = append(limit, 0)
	return query.KeyRange{Start: start, Limit: string(limit)}
}

// Compute a list of key ranges to be used by query's Table.Scan implementation.
func CompileKeyRanges(where *query_parser.WhereClause) *query.KeyRanges {
	if where == nil {
		return &query.KeyRanges{KeyRangeAll}
	}
...
func computeIntersection(lhs, rhs query.KeyRange) *query.KeyRange {
	...
	if (lhs.Start >= rhs.Start && compareStartToLimit(lhs.Start, rhs.Limit) < 0) ||
		...
		return &query.KeyRange{Start: start, Limit: limit}
	}
	return nil
}

func keyRangeIntersection(lhs, rhs *query.KeyRanges) *query.KeyRanges {
...
func collectKeyRanges(expr *query_parser.Expression) *query.KeyRanges {
	if IsExpr(expr.Operand1) { // then both operands must be expressions
		...
		if IsKey(expr.Operand1) && IsKey(expr.Operand2) {
			...
			case query_parser.Equal:
				return &query.KeyRanges{computeKeyRangeForSingleValue(expr.Operand2.Str)}
			case query_parser.GreaterThan:
				return &query.KeyRanges{query.KeyRange{Start: string(append([]byte(expr.Operand2.Str), 0)), Limit: MaxRangeLimit}}
			case query_parser.GreaterThanOrEqual:
				return &query.KeyRanges{query.KeyRange{Start: expr.Operand2.Str, Limit: MaxRangeLimit}}
			case query_parser.Like:
				return &query.KeyRanges{computeKeyRangeForLike(expr.Operand2.Prefix)}
			case query_parser.NotLike:
				return computeKeyRangesForNotLike(expr.Operand2.Prefix)
			case query_parser.LessThan:
				return &query.KeyRanges{query.KeyRange{Start: "", Limit: expr.Operand2.Str}}
			case query_parser.LessThanOrEqual:
				return &query.KeyRanges{query.KeyRange{Start: "", Limit: string(append([]byte(expr.Operand2.Str), 0))}}
			default: // case query_parser.NotEqual:
				return &query.KeyRanges{
					query.KeyRange{Start: "", Limit: expr.Operand2.Str},
					query.KeyRange{Start: string(append([]byte(expr.Operand2.Str), 0)), Limit: MaxRangeLimit},
				}
			}
			...
			switch expr.Operator.Type {
			case query_parser.Equal:
				return &query.KeyRanges{computeKeyRangeForSingleValue(expr.Operand1.Str)}
			case query_parser.GreaterThan:
				return &query.KeyRanges{query.KeyRange{Start: "", Limit: expr.Operand1.Str}}
			case query_parser.GreaterThanOrEqual:
				return &query.KeyRanges{query.KeyRange{Start: "", Limit: string(append([]byte(expr.Operand1.Str), 0))}}
			case query_parser.LessThan:
				return &query.KeyRanges{query.KeyRange{Start: string(append([]byte(expr.Operand1.Str), 0)), Limit: MaxRangeLimit}}
			case query_parser.LessThanOrEqual:
				return &query.KeyRanges{query.KeyRange{Start: expr.Operand1.Str, Limit: MaxRangeLimit}}
			default: // case query_parser.NotEqual:
				return &query.KeyRanges{
					query.KeyRange{Start: "", Limit: expr.Operand1.Str},
					query.KeyRange{Start: string(append([]byte(expr.Operand1.Str), 0)), Limit: MaxRangeLimit},
				}
			}
		} else {
...
func addKeyRange(keyRange query.KeyRange, keyRanges *query.KeyRanges) {
	handled := false
	...
	for i, r := range *keyRanges {
		...
		if (keyRange.Start >= r.Start && compareStartToLimit(keyRange.Start, r.Limit) <= 0) ||
			...
			var newKeyRange query.KeyRange
...
func checkLimitClause(db query.Database, l *query_parser.LimitClause) error {
	if l == nil {
		return nil
	}
	if l.Limit.Value < 1 {
		return query.NewErrLimitMustBeGe0(db.GetContext(), l.Limit.Off)
	}
	return nil
}
...
func checkResultsOffsetClause(db query.Database, o *query_parser.ResultsOffsetClause) error {
	if o == nil {
		return nil
	}
	if o.ResultsOffset.Value < 0 {
		return query.NewErrOffsetMustBeGe0(db.GetContext(), o.ResultsOffset.Off)
	}
	return nil
}
