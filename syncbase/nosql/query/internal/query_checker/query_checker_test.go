...
func (t invoiceTable) Scan(keyRanges query.KeyRanges) (query.KeyValueStream, error) {
	return nil, errors.New("unimplemented")
}

func (t customerTable) Scan(keyRanges query.KeyRanges) (query.KeyValueStream, error) {
	return nil, errors.New("unimplemented")
}
...
func TestKeyRanges(t *testing.T) {
	basic := []keyRangesTest{
		...
		},
		{
			...
			&query.KeyRanges{
				query.KeyRange{Start: "abc", Limit: appendZeroByte("abc")},
				query.KeyRange{Start: "def", Limit: appendZeroByte("def")},
			},
		},
		{
		...
		},
		{
			...
			&query.KeyRanges{
				query.KeyRange{Start: "foo", Limit: "goo"},
			},
		},
		{
			"select k, v from Customer where \"foo\" <= k and \"goo\" >= k",
			&query.KeyRanges{
				query.KeyRange{Start: "foo", Limit: appendZeroByte("goo")},
			},
		},
		{
			"select k, v from Customer where k <> \"foo\"",
			&query.KeyRanges{
				query.KeyRange{Start: "", Limit: "foo"},
				query.KeyRange{Start: appendZeroByte("foo"), Limit: ""},
			},
		},
		{
			"select k, v from Customer where k <> \"foo\" and k > \"bar\"",
			&query.KeyRanges{
				query.KeyRange{Start: appendZeroByte("bar"), Limit: "foo"},
				query.KeyRange{Start: appendZeroByte("foo"), Limit: ""},
			},
		},
		{
		...
		},
		{
			...
			&query.KeyRanges{
				query.KeyRange{Start: "", Limit: "bar"},
				query.KeyRange{Start: appendZeroByte("bar"), Limit: ""},
			},
		},
		{
			"select v from Customer where Type(v) = \"Foo.Bar\" and k >= \"100\" and k < \"200\" and v.foo > 50 and v.bar <= 1000 and v.baz <> -20.7",
			&query.KeyRanges{
				query.KeyRange{Start: "100", Limit: "200"},
			},
		},
		{
		...
		},
		{
			"select k, v from Customer where Type(v) = \"Foo.Bar\" and k like \"abc%\" limit 100 offset 200",
			&query.KeyRanges{
				query.KeyRange{Start: "abc", Limit: "abd"},
			},
		},
		{
		...
		},
		{
			...
			&query.KeyRanges{
				query.KeyRange{Start: "bar", Limit: "bas"},
				query.KeyRange{Start: "foo", Limit: "fop"},
			},
		},
		{
			...
			&query.KeyRanges{
				query.KeyRange{Start: "Foo", Limit: appendZeroByte("Foo")},
		...
		},
		{
			"select k, v from Customer where k like \"Foo\\\\%Bar\" or k like \"abc%\" limit 100 offset 200",
			&query.KeyRanges{
				query.KeyRange{Start: "Foo\\", Limit: "Foo]"},
				query.KeyRange{Start: "abc", Limit: "abd"},
			},
		},
		{
		...
		},
		{
			...
			&query.KeyRanges{
				query.KeyRange{Start: "", Limit: "002"},
				query.KeyRange{Start: "003", Limit: ""},
			},
		},
	}
...
func TestQueryCheckerErrors(t *testing.T) {
	basic := []parseSelectErrorTest{
		{"select a from Customer", query.NewErrInvalidSelectField(db.GetContext(), 7)},
		{"select v from Bob", query.NewErrTableCantAccess(db.GetContext(), 14, "Bob", errors.New("No such table: Bob"))},
		{"select k.a from Customer", query.NewErrDotNotationDisallowedForKey(db.GetContext(), 9)},
		{"select k from Customer where t.a = \"Foo.Bar\"", query.NewErrBadFieldInWhere(db.GetContext(), 29)},
		{"select v from Customer where a=1", query.NewErrBadFieldInWhere(db.GetContext(), 29)},
		{"select v from Customer limit 0", query.NewErrLimitMustBeGe0(db.GetContext(), 29)},
		{"select v.z from Customer where v.x like v.y", query.NewErrLikeExpressionsRequireRhsString(db.GetContext(), 31)},
		{"select v.z from Customer where k like \"a\\bc%\"", query.NewErrInvalidEscapedChar(db.GetContext(), 38)},
		{"select v from Customer where v.A > false", query.NewErrBoolInvalidExpression(db.GetContext(), 33)},
		{"select v from Customer where true <= v.A", query.NewErrBoolInvalidExpression(db.GetContext(), 34)},
		{"select v from Customer where Foo(\"2015/07/22\", true, 3.14157) = true", query.NewErrFunctionNotFound(db.GetContext(), 29, "Foo")},
		{"select v from Customer where nil is v.ZipCode", query.NewErrIsIsNotRequireLhsValue(db.GetContext(), 29)},
		{"select v from Customer where v.ZipCode is \"94303\"", query.NewErrIsIsNotRequireRhsNil(db.GetContext(), 42)},
		{"select v from Customer where v.ZipCode is 94303", query.NewErrIsIsNotRequireRhsNil(db.GetContext(), 42)},
		{"select v from Customer where v.ZipCode is true", query.NewErrIsIsNotRequireRhsNil(db.GetContext(), 42)},
		{"select v from Customer where v.ZipCode is 943.03", query.NewErrIsIsNotRequireRhsNil(db.GetContext(), 42)},
		{"select v from Customer where nil is not v.ZipCode", query.NewErrIsIsNotRequireLhsValue(db.GetContext(), 29)},
		{"select v from Customer where v.ZipCode is not \"94303\"", query.NewErrIsIsNotRequireRhsNil(db.GetContext(), 46)},
		{"select v from Customer where v.ZipCode is not 94303", query.NewErrIsIsNotRequireRhsNil(db.GetContext(), 46)},
		{"select v from Customer where v.ZipCode is not true", query.NewErrIsIsNotRequireRhsNil(db.GetContext(), 46)},
		{"select v from Customer where v.ZipCode is not 943.03", query.NewErrIsIsNotRequireRhsNil(db.GetContext(), 46)},
		{"select v from Customer where Type(v) = \"Customer\" and Year(v.InvoiceDate, \"ABC\") = 2015", query.NewErrLocationConversionError(db.GetContext(), 74, errors.New("unknown time zone ABC"))},
		{"select v from Customer where Type(v) = \"Customer\" and Month(v.InvoiceDate, \"ABC\") = 2015", query.NewErrLocationConversionError(db.GetContext(), 75, errors.New("unknown time zone ABC"))},
		{"select v from Customer where Type(v) = \"Customer\" and Day(v.InvoiceDate, \"ABC\") = 2015", query.NewErrLocationConversionError(db.GetContext(), 73, errors.New("unknown time zone ABC"))},
		{"select v from Customer where Type(v) = \"Customer\" and Hour(v.InvoiceDate, \"ABC\") = 2015", query.NewErrLocationConversionError(db.GetContext(), 74, errors.New("unknown time zone ABC"))},
		{"select v from Customer where Type(v) = \"Customer\" and Minute(v.InvoiceDate, \"ABC\") = 2015", query.NewErrLocationConversionError(db.GetContext(), 76, errors.New("unknown time zone ABC"))},
		{"select v from Customer where Type(v) = \"Customer\" and Second(v.InvoiceDate, \"ABC\") = 2015", query.NewErrLocationConversionError(db.GetContext(), 76, errors.New("unknown time zone ABC"))},
		{"select v from Customer where Type(v) = \"Customer\" and Now(v.InvoiceDate, \"ABC\") = 2015", query.NewErrFunctionArgCount(db.GetContext(), 54, "Now", 0, 2)},
		{"select v from Customer where Type(v) = \"Customer\" and Lowercase(v.Name, 2) = \"smith\"", query.NewErrFunctionArgCount(db.GetContext(), 54, "Lowercase", 1, 2)},
		{"select v from Customer where Type(v) = \"Customer\" and Uppercase(v.Name, 2) = \"SMITH\"", query.NewErrFunctionArgCount(db.GetContext(), 54, "Uppercase", 1, 2)},
		{"select Time() from Customer", query.NewErrFunctionArgCount(db.GetContext(), 7, "Time", 2, 0)},
		{"select Year(v.InvoiceDate, \"Foo\") from Customer where Type(v) = \"Invoice\"",
			query.NewErrLocationConversionError(db.GetContext(), 27, errors.New("unknown time zone Foo"))},
		{"select K from Customer where Type(v) = \"Invoice\"", query.NewErrDidYouMeanLowercaseK(db.GetContext(), 7)},
		{"select V from Customer where Type(v) = \"Invoice\"", query.NewErrDidYouMeanLowercaseV(db.GetContext(), 7)},
		{"select k from Customer where K = \"001\"", query.NewErrDidYouMeanLowercaseK(db.GetContext(), 29)},
		{"select v from Customer where Type(V) = \"Invoice\"", query.NewErrDidYouMeanLowercaseV(db.GetContext(), 34)},
		{"select K, V from Customer where Type(V) = \"Invoice\" and K = \"001\"", query.NewErrDidYouMeanLowercaseK(db.GetContext(), 7)},
	}
	...
	for _, test := range basic {
		...
		if syntaxErr != nil {
			...
			testErrOff, _ := exec.SplitError(test.err)
			errOff, _ := exec.SplitError(err)
