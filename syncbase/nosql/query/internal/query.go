...
func Exec(db query.Database, q string) ([]string, query.ResultStream, error) {
	...
	default:
		return nil, nil, query.NewErrExecOfUnknownStatementType(db.GetContext(), (*s).Offset(), reflect.TypeOf(*s).Name())
	}
}
...
func ComposeProjection(db query.Database, k string, v *vdl.Value, s *query_parser.SelectClause) []*vdl.Value {
...
// for a single row.
func ExecSelectSingleRow(db query.Database, k string, v *vdl.Value, s *query_parser.SelectStatement) []*vdl.Value {
...
type resultStreamImpl struct {
	...
	keyValueStream  query.KeyValueStream
	k               string
	v               *vdl.Value
	err             error
}
...
func (rs *resultStreamImpl) Advance() bool {
	...
	if err := rs.keyValueStream.Err(); err != nil {
		rs.err = query.NewErrKeyValueStreamError(rs.db.GetContext(), rs.selectStatement.Off, err)
	}
	return false
}
...
func execSelect(db query.Database, s *query_parser.SelectStatement) ([]string, query.ResultStream, error) {
	keyValueStream, err := s.From.Table.DBTable.Scan(*query_checker.CompileKeyRanges(s.Where))
	if err != nil {
		return nil, nil, query.NewErrScanError(db.GetContext(), s.Off, err)
	}
