...
type TableEntry struct {
	Name    string
	DBTable query.Table // Checker gets table from db and sets this.
	Node
}
...
func Parse(db query.Database, src string) (*Statement, error) {
	if len(src) > MaxStatementLen {
		return nil, query.NewErrMaxStatementLenExceeded(db.GetContext(), int64(0), MaxStatementLen, int64(len(src)))
	}
	...
	if token.Tok == TokEOF {
		return nil, query.NewErrNoStatementFound(db.GetContext(), token.Off)
	}
	if token.Tok != TokIDENT {
		return nil, query.NewErrExpectedIdentifier(db.GetContext(), token.Off, token.Value)
	}
	...
	case "select":
		...
		return &st, err
	default:
		return nil, query.NewErrUnknownIdentifier(db.GetContext(), token.Off, token.Value)
	}
}

// Parse the [currently] one and only supported statement: select.
func selectStatement(db query.Database, s *scanner.Scanner, token *Token) (Statement, *Token, error) {
	...
	if token.Tok != TokEOF {
		return nil, nil, query.NewErrUnexpected(db.GetContext(), token.Off, token.Value)
	}
...
func parseSelectClause(db query.Database, s *scanner.Scanner, token *Token) (*SelectClause, *Token, error) {
	...
	token = scanToken(s) // eat the select
	if token.Tok == TokEOF {
		return nil, nil, query.NewErrUnexpectedEndOfStatement(db.GetContext(), token.Off)
	}
	var err error
	// scan first selector
...
func parseSelector(db query.Database, s *scanner.Scanner, selectClause *SelectClause, token *Token) (*Token, error) {
	if token.Tok != TokIDENT {
		return nil, query.NewErrExpectedIdentifier(db.GetContext(), token.Off, token.Value)
	}
	...
	if token.Tok == TokLEFTPAREN {
		...
		if len(segment.Keys) != 0 {
			return nil, query.NewErrUnexpected(db.GetContext(), token.Off, token.Value)
		}
...
func parseSegment(db query.Database, s *scanner.Scanner, token *Token) (*Segment, *Token, error) {
	var segment Segment
	...
	for token.Tok == TokLEFTBRACKET {
		...
		if token.Tok != TokRIGHTBRACKET {
			return nil, nil, query.NewErrExpected(db.GetContext(), token.Off, "]")
		}
		token = scanToken(s)
	}
...
func parseFromClause(db query.Database, s *scanner.Scanner, token *Token) (*FromClause, *Token, error) {
	if strings.ToLower(token.Value) != "from" {
		return nil, nil, query.NewErrExpectedFrom(db.GetContext(), token.Off, token.Value)
	}
	...
	if token.Tok != TokIDENT {
		return nil, nil, query.NewErrExpectedIdentifier(db.GetContext(), token.Off, token.Value)
	}
...
func parseWhereClause(db query.Database, s *scanner.Scanner, token *Token) (*WhereClause, *Token, error) {
...
func parseParenthesizedExpression(db query.Database, s *scanner.Scanner, token *Token) (*Expression, *Token, error) {
...
func parseExpression(db query.Database, s *scanner.Scanner, token *Token) (*Expression, *Token, error) {
	if token.Tok == TokEOF {
		return nil, nil, query.NewErrUnexpectedEndOfStatement(db.GetContext(), token.Off)
	}
...
func parseLikeEqualExpression(db query.Database, s *scanner.Scanner, token *Token) (*Expression, *Token, error) {
...
func parseFunction(db query.Database, s *scanner.Scanner, funcName string, funcOffset int64, token *Token) (*Function, *Token, error) {
	...
	for token.Tok != TokRIGHTPAREN {
		...
		if token.Tok == TokCOMMA {
			...
			if token.Tok == TokRIGHTPAREN {
				...
				return nil, nil, query.NewErrExpectedOperand(db.GetContext(), token.Off, token.Value)
			}
...
func parseOperand(db query.Database, s *scanner.Scanner, token *Token) (*Operand, *Token, error) {
	if token.Tok == TokEOF {
		return nil, nil, query.NewErrUnexpectedEndOfStatement(db.GetContext(), token.Off)
	}
	var operand Operand
	...
	case TokINT:
		...
		if err != nil {
			return nil, nil, query.NewErrCouldNotConvert(db.GetContext(), token.Off, token.Value, "int64")
		}
		operand.Int = i
		token = scanToken(s)
	case TokFLOAT:
		...
		if err != nil {
			return nil, nil, query.NewErrCouldNotConvert(db.GetContext(), token.Off, token.Value, "float64")
		}
		operand.Float = f
		token = scanToken(s)
	case TokCHAR:
	...
	case TokMINUS:
		...
		case TokINT:
			...
			i, err := strconv.ParseInt("-"+token.Value, 0, 64)
			if err != nil {
				return nil, nil, query.NewErrCouldNotConvert(db.GetContext(), off, "-"+token.Value, "int64")
			}
			operand.Int = i
		case TokFLOAT:
			operand.Type = TypFloat
			f, err := strconv.ParseFloat("-"+token.Value, 64)
			if err != nil {
				return nil, nil, query.NewErrCouldNotConvert(db.GetContext(), off, "-"+token.Value, "float64")
			}
			operand.Float = f
		default:
			return nil, nil, query.NewErrExpected(db.GetContext(), token.Off, "int or float")
		}
		token = scanToken(s)
	default:
		return nil, nil, query.NewErrExpectedOperand(db.GetContext(), token.Off, token.Value)
...
func parseBinaryOperator(db query.Database, s *scanner.Scanner, token *Token) (*BinaryOperator, *Token, error) {
	if token.Tok == TokEOF {
		return nil, nil, query.NewErrUnexpectedEndOfStatement(db.GetContext(), token.Off)
	}
	var operator BinaryOperator
	...
	if token.Tok == TokIDENT {
		...
		case "not":
			...
			if token.Tok == TokEOF || (strings.ToLower(token.Value) != "equal" && strings.ToLower(token.Value) != "like") {
				return nil, nil, query.NewErrExpected(db.GetContext(), token.Off, "'equal' or 'like'")
			}
			switch strings.ToLower(token.Value) {
			case "equal":
			...
			token = scanToken(s)
		default:
			return nil, nil, query.NewErrExpectedOperator(db.GetContext(), token.Off, token.Value)
		}
	} else {
		switch token.Tok {
		case TokEQUAL:
...
func parseLogicalOperator(db query.Database, s *scanner.Scanner, token *Token) (*BinaryOperator, *Token, error) {
	if token.Tok == TokEOF {
		return nil, nil, query.NewErrUnexpectedEndOfStatement(db.GetContext(), token.Off)
	}
	var operator BinaryOperator
...
func parseLimitResultsOffsetClauses(db query.Database, s *scanner.Scanner, token *Token) (*LimitClause, *ResultsOffsetClause, *Token, error) {
	var err error
	var lc *LimitClause
...
func parseLimitClause(db query.Database, s *scanner.Scanner, token *Token) (*LimitClause, *Token, error) {
	var lc LimitClause
	lc.Off = token.Off
	token = scanToken(s)
	var err error
...
func parseResultsOffsetClause(db query.Database, s *scanner.Scanner, token *Token) (*ResultsOffsetClause, *Token, error) {
	var oc ResultsOffsetClause
	oc.Off = token.Off
	token = scanToken(s)
	var err error
...
func parseNonNegInt64(db query.Database, s *scanner.Scanner, token *Token) (*Int64Value, *Token, error) {
	...
	if token.Tok != TokINT {
		return nil, nil, query.NewErrExpected(db.GetContext(), token.Off, "positive integer literal")
	}
	var v Int64Value
	v.Off = token.Off
	var err error
