...
func (db *mockDB) GetTable(name string) (query.Table, error) {
	return nil, nil
}
...
func TestQueryParserErrors(t *testing.T) {
	basic := []parseSelectErrorTest{
		{"", query.NewErrNoStatementFound(db.GetContext(), 0)},
		{";", query.NewErrExpectedIdentifier(db.GetContext(), 0, ";")},
		{"foo", query.NewErrUnknownIdentifier(db.GetContext(), 0, "foo")},
		{"(foo)", query.NewErrExpectedIdentifier(db.GetContext(), 0, "(")},
		{"select foo.", query.NewErrExpectedIdentifier(db.GetContext(), 11, "")},
		{"select foo. from a", query.NewErrExpectedFrom(db.GetContext(), 17, "a")},
		{"select (foo)", query.NewErrExpectedIdentifier(db.GetContext(), 7, "(")},
		{"select from where", query.NewErrExpectedFrom(db.GetContext(), 12, "where")},
		{"create table Customer (CustRecord cust_pkg.Cust, primary key(CustRecord.CustID))", query.NewErrUnknownIdentifier(db.GetContext(), 0, "create")},
		{"select foo from Customer where (A=123 or B=456) and C=789)", query.NewErrUnexpected(db.GetContext(), 57, ")")},
		{"select foo from Customer where ((A=123 or B=456) and C=789", query.NewErrUnexpectedEndOfStatement(db.GetContext(), 58)},
		{"select foo from Customer where (((((A=123 or B=456 and C=789))))", query.NewErrUnexpectedEndOfStatement(db.GetContext(), 64)},
		{"select foo from Customer where (A=123 or B=456) and C=789)))))", query.NewErrUnexpected(db.GetContext(), 57, ")")},
		{"select foo from Customer where", query.NewErrUnexpectedEndOfStatement(db.GetContext(), 30)},
		{"select foo from Customer where ", query.NewErrUnexpectedEndOfStatement(db.GetContext(), 31)},
		{"select foo from Customer where )", query.NewErrExpectedOperand(db.GetContext(), 31, ")")},
		{"select foo from Customer where )A=123 or B=456) and C=789", query.NewErrExpectedOperand(db.GetContext(), 31, ")")},
		{"select foo from Customer where ()A=123 or B=456) and C=789", query.NewErrExpectedOperand(db.GetContext(), 32, ")")},
		{"select foo from Customer where (A=123 or B=456) and C=789)", query.NewErrUnexpected(db.GetContext(), 57, ")")},
		{"select foo bar from Customer", query.NewErrExpectedFrom(db.GetContext(), 11, "bar")},
		{"select foo from Customer Invoice", query.NewErrUnexpected(db.GetContext(), 25, "Invoice")},
		{"select (foo) from (Customer)", query.NewErrExpectedIdentifier(db.GetContext(), 7, "(")},
		{"select foo, bar from Customer where a = (b)", query.NewErrExpectedOperand(db.GetContext(), 40, "(")},
		{"select foo, bar from Customer where a = b and (c) = d", query.NewErrExpectedOperator(db.GetContext(), 48, ")")},
		{"select foo, bar from Customer where a = b and c =", query.NewErrUnexpectedEndOfStatement(db.GetContext(), 49)},
		{"select foo, bar from Customer where a = ", query.NewErrUnexpectedEndOfStatement(db.GetContext(), 40)},
		{"select foo, bar from Customer where a", query.NewErrUnexpectedEndOfStatement(db.GetContext(), 37)},
		{"select", query.NewErrUnexpectedEndOfStatement(db.GetContext(), 6)},
		{"select a from", query.NewErrUnexpectedEndOfStatement(db.GetContext(), 13)},
		{"select a from b where c = d and e =", query.NewErrUnexpectedEndOfStatement(db.GetContext(), 35)},
		{"select a from b where c = d and f", query.NewErrUnexpectedEndOfStatement(db.GetContext(), 33)},
		{"select a from b where c = d and f *", query.NewErrExpectedOperator(db.GetContext(), 34, "*")},
		{"select a from b where c <", query.NewErrUnexpectedEndOfStatement(db.GetContext(), 25)},
		{"select a from b where c not", query.NewErrExpected(db.GetContext(), 27, "'equal' or 'like'")},
		{"select a from b where c not 8", query.NewErrExpected(db.GetContext(), 28, "'equal' or 'like'")},
		{"select x from y where a and b = c", query.NewErrExpectedOperator(db.GetContext(), 24, "and")},
		{"select v from Customer limit 100 offset a", query.NewErrExpected(db.GetContext(), 40, "positive integer literal")},
		{"select v from Customer limit -100 offset 5", query.NewErrExpected(db.GetContext(), 29, "positive integer literal")},
		{"select v from Customer limit 100 offset -5", query.NewErrExpected(db.GetContext(), 40, "positive integer literal")},
		{"select v from Customer limit a offset 200", query.NewErrExpected(db.GetContext(), 29, "positive integer literal")},
		{"select v from Customer limit", query.NewErrUnexpectedEndOfStatement(db.GetContext(), 28)},
		{"select v from Customer, Invoice", query.NewErrUnexpected(db.GetContext(), 22, ",")},
		{"select v from Customer As Cust where foo = bar", query.NewErrUnexpected(db.GetContext(), 23, "As")},
		{"select 1abc from Customer where foo = bar", query.NewErrExpectedIdentifier(db.GetContext(), 7, "1")},
		{"select v from Customer where Foo(1,) = true", query.NewErrExpectedOperand(db.GetContext(), 35, ")")},
		{"select v from Customer where Foo(,1) = true", query.NewErrExpectedOperand(db.GetContext(), 33, ",")},
		{"select v from Customer where Foo(1, 2.0 = true", query.NewErrUnexpected(db.GetContext(), 40, "=")},
		{"select v from Customer where Foo(1, 2.0 limit 100", query.NewErrUnexpected(db.GetContext(), 40, "limit")},
		{"select v from Customer where v is", query.NewErrUnexpectedEndOfStatement(db.GetContext(), 33)},
		{"select v from Customer where v = 1.0 is k = \"abc\"", query.NewErrUnexpected(db.GetContext(), 37, "is")},
		{"select v as from Customer", query.NewErrExpectedFrom(db.GetContext(), 17, "Customer")},
	}
...
func TestStatementSizeExceeded(t *testing.T) {
	...
	expectedErr := query.NewErrMaxStatementLenExceeded(db.GetContext(), int64(0), query_parser.MaxStatementLen, int64(len(q)))
