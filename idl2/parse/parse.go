// Package parse provides utilities to parse idl files into a parse tree.  The
...
// Opts specifies idl parsing options.
type Opts struct {
	ImportsOnly bool // Only parse imports; skip everything else.
}

// Parse takes an idl base file name, the contents of the file src, and the
// accumulated errors, and parses the idl into a parse.File, containing the
...
func Parse(baseFileName string, src io.Reader, opts Opts, errs *idl2.Errors) *File {
	if errs == nil {
	...
	lex.attachComments()
	if !opts.ImportsOnly {
		idl2.Vlog.Printf("PARSE RESULTS\n\n%v\n\n", lex.idlFile)
	}
...
func newLexer(baseFileName string, src io.Reader, opts Opts, errs *idl2.Errors) *lexer {
	l := &lexer{opts: opts, errs: errs, idlFile: &File{BaseName: baseFileName}}
	l.comments.init()
	l.scanner.Init(src)
...
// lexIDLFile retrieves the idl2.File parse result from the yyLexer interface.
...
func (l *lexer) posErrorf(pos Pos, format string, v ...interface{}) {
	...
	l.errs.Errorf(l.idlFile.BaseName+":"+posstr+" "+format, v...)
}
