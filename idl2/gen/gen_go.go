// Package gen provides functions to generate code from compiled IDL packages.
package gen
...
type userImport struct {
	...
	Path  string // Path of the import; e.g. "veyron2/idl2"
...
func (u userImports) lookupImport(pkgpath string) string {
	...
	panic(fmt.Errorf("idl: import path %q not found in %v", pkgpath, u))
}
...
func typeGo(data goData, t *val.Type) string {
	...
	case val.Map:
		...
		return "map[" + key + "]" + elem
	default:
		panic(fmt.Errorf("idl: unhandled type %v %v", t.Kind(), t))
	}
}
...
func valueGo(v *val.Value) string {
	...
	panic(fmt.Errorf("idl: valueGo unhandled type %v %v", v.Kind(), v.Type()))
}
...
func bitlen(kind val.Kind) int {
	...
	panic(fmt.Errorf("idl: bitLen unhandled kind %v", kind))
}
...
func typeDefsCode(td []idl2.Any) string {
	var buf bytes.Buffer
	buf.WriteString("[]_gen_idl2.Any{\n")
	for _, wt := range td {
		switch t := wt.(type) {
