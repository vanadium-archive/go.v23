...
func TestParseAndCompile(t *testing.T) {
	tests := []struct {
		...
		{"test1", f{"1.idl": pkg1file1, "2.idl": pkg1file2}, "", expectPkg1},
	}
	for _, test := range tests {
		path := path.Join("a/b", test.name)
		buildPkg := idltest.FakeBuildPackage(test.name, path, test.files)
		env := compile.NewEnv(-1)
		pkg := build.CompilePackage(buildPkg, env)
		idltest.ExpectResult(t, env.Errors, test.name, test.errRE)
		if pkg == nil {
			continue
		}
