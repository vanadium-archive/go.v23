...
func (td typeDefiner) Define() {
	for _, b := range td.builders {
		...
		default:
			...
			panic(fmt.Errorf("idl: unhandled parse.Type %T %#v", pt, pt))
		}
	}
}
...
var (
	...
	GlobalFile    = &File{BaseName: "global.idl"}

	// Built-in types defined by the compiler.
	// TODO(toddw): Remove byte from the IDL language.
	TypeByte = val.NamedType("byte", val.Uint32Type)
	// TODO(toddw): Represent error in a built-in IDL file.
