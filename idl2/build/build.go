// Package build provides utilities to collect idl build information, and
...
import (
	...
	"veyron/lib/toposort"
	"veyron2/idl2"
	"veyron2/idl2/compile"
	"veyron2/idl2/parse"
)

// Package represents the build information for an idl package.
type Package struct {
	...
	// E.g. "veyron/rt/base"
	Path string
	// BaseFileNames is an unordered list of base idl file names for this
...
// via resolvePkgPath.
func newPackage(dir string, mode missingMode, errs *idl2.Errors) *Package {
	pkg := &Package{Dir: dir, OpenFilesFunc: openFiles}
	if err := pkg.initBaseFileNames(); err != nil {
		errs.Errorf("Couldn't init idl file names in package dir %v, %v", pkg.Dir, err)
		return nil
	}
	if len(pkg.BaseFileNames) == 0 {
		if mode == missingIsError {
			errs.Errorf("No idl files in dir %v", pkg.Dir)
		}
		return nil
	}
	return pkg
...
func (p *Package) initBaseFileNames() error {
	idl2.Vlog.Printf("Looking for idl files in package dir %v", p.Dir)
...
// idl2.Package information and write it out as compiler-generated artifacts,
...
func toAbs(dirs []string, errs *idl2.Errors) (ret []string) {
	for _, d := range dirs {
...
func newDepSorter(errs *idl2.Errors) *depSorter {
	return &depSorter{
...
func (ds *depSorter) resolvePkgPath(pkgPath string) (*Package, bool) {
	...
	for _, srcDir := range ds.srcDirs {
		...
		if pkg, isNew := ds.resolvePkgDir(candidateDir, missingIsOk); pkg != nil {
			idl2.Vlog.Printf("Resolved pkg path %v to abs dir %v", pkgPath, pkg.Dir)
...
// transitive order.
func TransitivePackages(cmdLineArgs []string, errs *idl2.Errors) []*Package {
...
func ParsePackage(pkg *Package, opts parse.Opts, errs *idl2.Errors) (pfiles []*parse.File) {
	idl2.Vlog.Printf("Parsing package %s %q, dir %s", pkg.Name, pkg.Path, pkg.Dir)
	files, err := pkg.OpenFiles()
	if err != nil {
		errs.Errorf("Couldn't open idl files %v, %v", pkg.BaseFileNames, err)
		return nil
	}
