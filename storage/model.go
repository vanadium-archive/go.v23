...
//
// - Use BindObject(path) to get an Entry in the store.  An Entry is just a
//   reference, it does not mean that there is a value associated with the
//   Entry.  Use Put(v) to store a value.
//
// - Use transactionRoot.CreateTransaction() to create a transaction.
//   Transactions acquire a snapshot of the state the first time they are used,
//   and may not span multiple store instances.
//
...
//
//       tid, err := st.BindTransactionRoot("/").CreateTransaction()
//       tname := naming.Join("/", tid)
//       d := st.BindObject(tname)
//       d.Put(&Dir{ID: 10})
//       y, err := st.BindObject(naming.Join(tname, "/Entries/Yosemite"))
//       y.Put(&Album{ID: 11, Title: "Yosemite 2013", Images: make(map[string]Name)})
//       img, err := st.Bind(naming.Join(tname, "/Entries/Yosemite/Images/A"))
//       img.Put(&Image{ID: 37, Comment: "Waterfall", Content: "global/media/xxx.jpg"})
//       img, err : st.Bind(naming.Join(tname, "/Entries/Yosemite/Images/B"))
//       img.Put(&Image{ID: 21, Comment: "Jason", Content: "global/media/yyy.jpg"})
//       st.BindTransaction(tname).Commit()
//
...
//
// - Hard links are supported.  To add a hard link, use a Bind()/Put() to add a
//   value with each of its paths.
//
//       // Add a link to the image by date.
//       tid, err := st.BindTransactionRoot("/").CreateTransaction()
//       tname := naming.Join("/", tid)
//       img1, err := st.BindObject(naming.Join(tname, "/Entries/Yosemite/Images/A"))
//       v, err := img1.Get()
//       img2, err := st.BindObject(naming.Join(tname, "/Entries/Wallpaper"))
//       img2.Put(v)
//       st.BindTransaction(tname).Commit()
//
...
//
// - For sharing, data is replicated via replication groups.  Directories are
//   replicated by adding a ReplicationGroupAttr attribute.
//
...
//
// - If an entry was modified concurrently in two replicas, the conflict is
//   detected, and the entry is passed to a conflict resolver.
//
...
type TransactionRoot interface {
	...
	CreateTransaction(ctx context.T, opts ...TransactionOpt) (string, error)
}
...
type Store interface {
	// BindObject returns the Object associated with a path.
	BindObject(path string) Object

	// BindTransactionRoot returns a TransactionRoot for the given path.
	BindTransactionRoot(path string) TransactionRoot
