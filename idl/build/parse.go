...
// with entrypoint yyParse.  The result of the parse is the simple idl.File
...
// ParsePackageImports takes a map of files from idl base filename to the
// contents of the idl file source, and the parsing environment env, and returns
// the package name and import package paths.
func ParsePackageImports(files map[string]io.ReadCloser, env *Env) (pkgName string, depPkgPaths []string) {
	var firstFileName string
	for baseFileName, src := range files {
		file := parseFile(baseFileName, src, parseOpts{ImportsOnly: true}, env)
		if file != nil {
			if len(pkgName) == 0 {
				firstFileName = file.BaseName
				pkgName = file.PackageName
			} else if pkgName != file.PackageName {
				env.Errors.Errorf("Files in the same directory must be in the same package; %v has package %v, but %v has package %v", firstFileName, pkgName, file.BaseName, file.PackageName)
				continue
			}
			for _, imp := range file.Imports {
				depPkgPaths = append(depPkgPaths, imp.Path)
			}
		}
	}
	return
...
// parseFile takes an idl base fileName, the contents of the file src, and the
// parsing environment env, and parses the idl into an idl.File.  Returns nil if
// any errors are encountered, with env.Errors containing more information.
// Otherwise returns the parsed idl.File.
func parseFile(baseFileName string, src io.Reader, opts parseOpts, env *Env) *File {
	if env == nil {
		log.Fatal("Nil environment specified for parseFile")
	}
	startErrs := env.Errors.NumErrors()
	lex := newLexer(baseFileName, src, opts, env)
	if errCode := yyParse(lex); errCode != 0 {
		env.Errors.Errorf("%s yyParse returned error code %v", baseFileName, errCode)
	}
	lex.attachComments()
	if !opts.ImportsOnly {
		vlog.Printf("PARSE RESULTS\n\n%v\n\n", lex.idlFile)
	}
	if startErrs != env.Errors.NumErrors() {
		return nil
	}
	return lex.idlFile
}
...
// call lexIDLFile(yylex) to convert from the yyLexer interface back to the
...
func newLexer(baseFileName string, src io.Reader, opts parseOpts, env *Env) *lexer {
	l := &lexer{opts: opts, env: env, idlFile: &File{BaseName: baseFileName}}
	l.comments.init()
	l.scanner.Init(src)
...
// lexIDLFile retrieves the idl.File parse result from the yyLexer interface.
// This is called in the yacc rules to fill in the parse result.
func lexIDLFile(yylex yyLexer) *File {
	return yylex.(*lexer).idlFile
}
...
func (cm *commentMap) addComment(tok token) {
	if !cm.cur.update(tok) {
		cm.endBlock()
		if !cm.cur.update(tok) {
			panic(fmt.Errorf("idl: couldn't update current comment block with token %v", tok))
		}
	}
...
func (l *lexer) attachComments() {
	...
	for _, td := range l.idlFile.TypeDefs {
		attachTypeComments(td.Base, &l.comments, true)
	}
	for _, cd := range l.idlFile.ConstDefs {
		cd.DocSuffix = l.comments.getDocSuffix(cd.Pos)
	}
	for _, ei := range l.idlFile.ErrorIDs {
		ei.DocSuffix = l.comments.getDocSuffix(ei.Pos)
	}
	for _, iface := range l.idlFile.Interfaces {
		for _, component := range iface.Components {
			component.SetDocSuffix(l.comments.getDocSuffix(component.GetPos()))
		}
	}
	// Now attach the docs - these occur on the line immediately before.
	l.idlFile.PackageDoc = l.comments.getDoc(l.idlFile.PackagePos)
	for _, td := range l.idlFile.TypeDefs {
		td.Doc = l.comments.getDoc(td.Pos)
		attachTypeComments(td.Base, &l.comments, false)
	}
	for _, cd := range l.idlFile.ConstDefs {
		cd.Doc = l.comments.getDoc(cd.Pos)
	}
	for _, ei := range l.idlFile.ErrorIDs {
		ei.Doc = l.comments.getDoc(ei.Pos)
	}
	for _, iface := range l.idlFile.Interfaces {
		iface.Doc = l.comments.getDoc(iface.Pos)
		for _, component := range iface.Components {
			component.SetDoc(l.comments.getDoc(component.GetPos()))
		}
	}
}
...
// handleComplex handles checking if a number is imaginary "[number]i" by peeking ahead.
func (l *lexer) handleComplex(tok token, lval *yySymType) bool {
	if l.scanner.Peek() != 'i' {
		return false
	}
	l.scanner.Next()
	...
	lval.cpxpos.pos = tok.pos
	lval.cpxpos.cpx = bigCmplx{
		re: bigRatZero,
		im: rat,
	}
	return true
...
func (l *lexer) posErrorf(pos Pos, format string, v ...interface{}) {
	...
	l.env.Errors.Errorf(l.idlFile.BaseName+":"+posstr+" "+format, v...)
}
