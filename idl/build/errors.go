...
// Errorf takes the same args as fmt.Printf, and adds the error described by
// format and v to the buffer.  Returns true iff we're still under the MaxErrors
// cutoff.
func (e *Errors) Errorf(format string, v ...interface{}) bool {
	if e.num == e.MaxErrors {
		return false
	}
	// TODO(toddw): The Go compiler complains if we concatenate the format
	// strings; it seems like it's incorrectly attempting static analysis:
	//    fmt.Sprintf("#%v "+format+"\n", e.num, v...)
	msg1 := "#" + strconv.Itoa(e.num) + " " + fmt.Sprintf(format, v...) + "\n"
	e.buf.WriteString(msg1)
	if e.num++; e.num == e.MaxErrors {
		msg2 := fmt.Sprintf("...stopping after %v error(s)...\n", e.num)
